...
def process_video(video_path, model_path='yolov8m-pose.pt', output_video_path=None, output_json_path=None):
    ...
    if not (0 < jump_start_index_hip < jump_peak_index_hip < jump_end_index_hip < frame_count):
         ...
         print(f"Status: Failed (Invalid jump phase indices)")
         print(f"Calculated Jump Height: 0.0 cm")
         print(f"Calculated Flight Time: 0.000 s")
         print(f"Calculated Velocity: 0.00 m/s")
         print(f"Calculated Knee Angle (at lowest point): 0.0 degrees")
         print(f"Calculated Hip Angle (at peak): 0.0 degrees")
         print("-----------------------------")
         return
    ...
    if jump_height_cm < 2 or jump_height_cm > 140 or time_in_air_s <= 0: # time_in_air must be positive
        ...
        knee_angle_deg = 0.0
        hip_angle_deg = 0.0
        # Early exit after printing error message and resetting values
        print("\n--- Jump Analysis Results ---")
        print(f"Status: Failed (Metrics out of range or low confidence)")
        print(f"Calculated Jump Height: {jump_height_cm:.1f} cm")
        print(f"Calculated Flight Time: {time_in_air_s:.3f} s")
        print(f"Calculated Velocity: {velocity_mps:.2f} m/s")
        print(f"Calculated Knee Angle (at lowest point): {knee_angle_deg:.1f} degrees")
        print(f"Calculated Hip Angle (at peak): {hip_angle_deg:.1f} degrees")
        print("-----------------------------")
        return
    ...
    print(f"Calculated Jump Height: {jump_height_cm:.1f} cm")
    print(f"Calculated Flight Time: {time_in_air_s:.3f} s")
    print(f"Take-off Velocity (est.): {velocity_mps:.2f} m/s")
    print(f"Knee Angle (at lowest point): {knee_angle_deg:.1f} degrees (Frame {knee_angle_index})")
    print(f"Hip Angle (at peak): {hip_angle_deg:.1f} degrees (Frame {jump_peak_index_hip})")
    print("-----------------------------")


# --- Argument Parsing and Execution ---
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Analyze jump performance from a video using YOLO11 Pose.")
    parser.add_argument("--video_path", help="Path to the input video file.", default="test.mp4") # Changed to positional argument
    parser.add_argument("-m", "--model", default="yolo11m-pose.pt", help="Path to the YOLOv8 Pose model file (e.g., yolov8m-pose.pt).")
    parser.add_argument("-o", "--save_video", default='test_output.mp4', help="Optional path to save the annotated video (e.g., output.mp4).")
    parser.add_argument("-j", "--save_json", default='test_results.json', help="Optional path to save raw detection results as JSON (e.g., results.json).")
    # Add arguments for thresholds? e.g., confidence threshold
    ...
    if not os.path.exists(args.model):
         ...
         # Example: Download command hint (might need adjustment based on ultralytics version)
         # print("You might be able to download it using: yolo predict model=yolov8m-pose.pt source=0")
    # Check if video file exists
    elif not os.path.exists(args.video_path):
         print(f"Error: Video file not found at {args.video_path}")
    else:
        # Pass the output video path and JSON path to the processing function
        process_video(args.video_path, args.model, args.save_video, args.save_json) 